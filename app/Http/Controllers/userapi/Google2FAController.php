  ...
  /**
    ...
    public function resetG2faUser(Request $request){
        try {
            ...
            // dd($request->otp,$otpdata,$user->id,md5($request->otp));
            if (!empty($otpdata)) {
                if ($otpdata->otp_status == 0) {
                    if($today > $otpdata->otpexpire){
                        ...
                        if ($today > $user->g2fa_token_expire) {
                            $arrStatus = Response::HTTP_NOT_FOUND;
                            $arrCode = Response::$statusTexts[$arrStatus];
                            $arrMessage = 'G2FA token expired, please try again.';
                            return sendResponse($arrStatus, $arrCode, $arrMessage, '');
                        } else {
    ...
    public function onMailLinkClick(){
