...
class MarketingController extends Controller
{
    ...
    public function addMarketingTools(Request $request){
        try {
            ...
            //$add_fund_otp_status = $adminOtpStatusData->add_fund_otp_status;
            // if ($add_fund_otp_status == 1) {
    ...
    public function uploadmultiple(Request $request){
        ...
        $input = $request->all();    
        $filePath = public_path('user_files/images');
        ...
        if(is_file($filePath)){
            ...
            rename($filePath,public_path('user_files/images/'.$newfilename));
            $result=$newfilename;
        }
        $info = "Upload OK";
        $ok =1;

        return response()->json(["ok"=>$ok, "info"=>$info,'result'=>$result]);
    }


    function uploadmultiplesubmit(Request $request){

    

        if(!empty($request->uploadfiles)) {
            $i =0;
            foreach($request->uploadfiles as $upload_image){
               
                if($upload_image!=''){


                $localFilePath =  public_path('user_files/images/'.$upload_image);
                $userExist = Auth::user();
                ...
                if($userExist){                  
                    if (!empty($userExist)) {
                          ...
                          //  $marketTool->tool_name = $request->tool_name;
                            ...
                            $res['success'] = 1;
                            $res['message'] = 'Records has been Updated';
    ...
    function getVideos(Request $request){
        ...
        $arrInput  = $request->all();
        $query = MarketTool::select( 'tbl_marketing_tools.tool_name',  'tbl_marketing_tools.tool_url')->where('tbl_marketing_tools.tool_type', $request->tool_type)->get();
        
        $responseData = array();
        foreach($query as $MarketToolVideos){

           $tool_name = $MarketToolVideos['tool_name'];
           $tool_url = $MarketToolVideos['tool_url'];

            preg_match('/src="([^"]+)"/', $tool_url, $match);
            $url = $match[1];
