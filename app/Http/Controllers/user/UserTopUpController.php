...
//        }
    ...
    public function withdrawROIWallet(Request $request)
    {
        try {
            ...
            // dd($otpdata['status']);
            if ($otpdata['status'] == 200) {
                $dash = Dashboard::where('id', $id)->select('roi_income', 'roi_income_withdraw','roi_wallet', 'roi_wallet_withdraw', 'working_wallet', 'working_wallet_withdraw')
                    ->first();
...
//                 $roi_income_balance = $dash->roi_income - $dash->roi_income_withdraw;
                ...
                if ($request->working_wallet <= $roi_income_balance) {
                    ...
                    $updateData['roi_wallet_withdraw'] = DB::raw("roi_wallet_withdraw + " . $request->working_wallet);
