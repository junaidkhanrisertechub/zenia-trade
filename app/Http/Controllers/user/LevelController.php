...
class LevelController extends Controller {
	...
	public function getLevelsViewTreeManualProductBase(Request $request) {
		...
		$lCount = User::where('tbl_users.ref_user_id', Auth::user()->id)->where('tbl_users.position', "1")->count();
		$rCount = User::where('tbl_users.ref_user_id', Auth::user()->id)->where('tbl_users.position', "2")->count();
		...
		if (!empty($objUser)) {
			...
			return view('user/tree/treeview',$arrData);
