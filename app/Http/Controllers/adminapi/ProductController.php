...
class ProductController extends Controller
{
	...
	public function addTopBlade(){
		return view('admin.AddTopUpBlade');
	}
	...
	{	
		...
		if ($validator->fails()) {
			...
			$objTopup = Topup::select('roi_stop_status','pin','duration','id','roi_status','type')->where('pin', $request->sr_no)->first();
			if (!empty($objTopup)) {
				if($objTopup->type == 5){
