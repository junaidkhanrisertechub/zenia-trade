...
// dd($totalSummary[0]->amount);
	...
	public function getWithdrwalConfirmedTotals($request)
	{
		...
		$query = WithdrawalConfirmed::join('tbl_users as tu', 'tu.id', '=', 'tbl_withdrwal_confirmed.id')
			->selectRaw('COALESCE(ROUND(SUM(tbl_withdrwal_confirmed.amount),5),0) as amount,COALESCE(ROUND(SUM(tbl_withdrwal_confirmed.deduction),5),0) as deduction,COALESCE(ROUND(SUM(tbl_withdrwal_confirmed.transaction_fee),5),0) as transaction_fee,tbl_withdrwal_confirmed.entry_time')
			->where('tbl_withdrwal_confirmed.status', 1)->first; //confirmed status
